...
/**
 * Date : 02.08.2023
 * Problem : Fish CF-R16-Div2 https://www.youtube.com/redirect?event=video_description&redir_token=QUFFLUhqbFgyd1FsVE9xbnpNZ2FzSHdzZEl4OEpVS1FjZ3xBQ3Jtc0trX3lUMFgxa3phbThaZFdyRWFiN3cxUDZwQXVicm9aWmxSN3lmQXBzd3BWbGpzY3l6Nmd2Ulg3UWpxNHE0RDRwaDVHVy1QaEVaemNFTzlVVTg3bkdRM3JnVVJSVVM3amFiR25obzNxbDVvNm8xU00xaw&q=https%3A%2F%2Fdocs.google.com%2Fdocument%2Fd%2F1zuw8hBXHsiTYTH8u986fQhn8TWfpOk9BQBIRH3lo_W8%2Fedit%3Fusp%3Dsharing&v=685x-rzOIlY
 * dp[mask][i] : minimum cost to assign job 'i' to jth person hidden inside mask(check jth bit on)
 * dp[mask][i] = min {cost[j][i]+dp[mask/j][i+1], for all valid j}
 *
 * TC : O(N*2^N)
...
double prob[20][20];
double dp[1 << 19];
int solve(int bitMask, int &n)
{
    int alive = __builtin_popcount(bitMasj)
}
